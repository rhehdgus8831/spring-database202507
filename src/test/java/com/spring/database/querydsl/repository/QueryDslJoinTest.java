...
public class QueryDslJoinTest {
    ...
    void innerJoinTest () {
        ...
        List<Idol> idolList = factory
                .select(idol)
                .from(idol)
                // inner Join의 첫번째 파라미터는 from절에 쓴 엔터티의 연관 관계 객체
                // 두번쨰 파라미터는 실제로 조인할 엔터티
    ...
    void albumTest() {
        /*
            ...
            FROM tbl_idol I
            JOIN tbl_group G
            ON I.idol_id = G.group_id
            JOIN tbl_group A
            ON G.group_id = A.album_id
        ...
        */
        ...
        List<Tuple> idolList = factory
                ...
                .from(idol)
                // 아 idol은 알고 있는데 idol에 있는 group이 QGroup이랑 같은거구나
                .innerJoin(idol.group, group)
                // 아 group은 알고 있는데 album은 뭐야?
                // group에 있는 albums가 album이야
